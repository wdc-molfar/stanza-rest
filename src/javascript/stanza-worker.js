...
const restartWorker = () =>  new Promise( (resolve, reject) => {
	...
	console.log(`Restart ../python/${config.python.script.stanza}`)
	...
	worker.getShells()[0].shell.on("close", (code, signal) => {
		...
		// worker.terminate()
		reject(`Instance: ${(worker) ? worker.id : 'null'} for ../python/${config.python.script.stanza} not started. ${m}`)
		worker = null
	})
	


	console.log(`Instance: ${worker.id} for ../python/${config.python.script.stanza}`)
...
const Worker = class extends Bridge {
	...
	constructor(){
		super(config.python)
		this.id = uuid()
		this.use("__stanza",path.resolve(__dirname,`../python/${config.python.script.stanza}`))
	}

	async request(data) {
		try {
			// console.log("worker request", data)
			let result = await this.__stanza(data)
			// console.log(result)
